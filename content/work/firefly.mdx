...
Firefly's retry model is configurable per tenant and per event type. The defaults use exponential backoff with jitter:

```
delay = base * 2^attempt + rand(0, base)
```

The jitter matters. Without it, a batch of failed deliveries retries in lockstep, creating a thundering herd that overwhelms a recovering endpoint. With jitter, retries spread out naturally.
...
Firefly supports OAuth for authenticating outbound webhook calls. Tokens are fetched and cached, refreshed automatically before expiry. Recipients that require signed payloads get HMAC signatures on every request. The auth configuration lives in Pickle, so it can be updated without a code change.
