...
service payments {
  vendor stripe {
    ...
    encrypt body aes-256
    retry 3 exponential
    on_failure fallback razorpay
  }
}
```
...
The failover mechanism is deterministic from the config perspective — teams declare `on_failure fallback <vendor>` — but the scoring system means the "primary" vendor is always the currently best-performing one.

## Custom code execution

Some business logic doesn't fit cleanly into declarative config. For those cases, Octopus supports custom code plugins using Go's native plugin system. Teams can write Go code that gets loaded at runtime and executed in isolation — no separate containers, no gRPC calls, no serialization overhead.
...
It handles service onboarding — giving teams a UI to register their services and configure their DSL without touching files directly. It shows live proxy metrics: request volume per vendor, error rates, scoring history, active failover states. It also provides an audit log of configuration changes.

The frontend is React. The backend talks to Octopus over gRPC.

---

The biggest lesson from building Octopus was that the DSL design is the hardest part. Getting the abstraction level right — expressive enough to cover real use cases, simple enough that teams can read and write it without documentation — took several iterations. The execution engine was comparatively straightforward once the language had the right primitives.
